...
def run_benchmark(SEQLEN, TOKEN_LEN, dtype):
    paddle.seed(42)
    # Parameters based on the context provided
    expert_num = 16
    topk = 8
    
    # Iterate over both configurations
    print("\n\n" + "=" * 60)
    print(f"Configuration: SEQLEN={SEQLEN}, TOKEN_LEN={TOKEN_LEN}, dtype={dtype}")
    print(f"               expert_num={expert_num}, topk={topk}")
    print("-" * 60)

    # Data preparation (must be done per config as scale shape depends on it)
    (
        hidden_states,
        scale,
        expert_routemap_topk,
    ...
    # print("expert_prob_topk", expert_prob_topk.shape, expert_prob_topk.dtype)
    # print("len(tokens_per_expert)", len(tokens_per_expert))
    ...
    for dtype in dtypes:
        for SEQLEN in SEQLENS:
            for TOKEN_LEN in TOKEN_LENS:
                run_benchmark(SEQLEN, TOKEN_LEN, dtype)
    
    # 保存结果到CSV
    if results:
        csv_file = 'moe_permute_paddle_h.csv'
        fieldnames = [
