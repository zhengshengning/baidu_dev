...
std::vector<paddle::Tensor> fused_stack_transpose_quant_custom(
...
PD_BUILD_OP(fused_stack_quant_custom)
    .Inputs({paddle::Vec("X")})
    .Outputs({"output", "scale"})
    .SetKernelFn(PD_KERNEL(fused_stack_transpose_quant_custom<false>));

PD_BUILD_OP(fused_stack_transpose_quant_custom)
    .Inputs({paddle::Vec("X")})
    .Outputs({"output", "scale"})
    .SetKernelFn(PD_KERNEL(fused_stack_transpose_quant_custom<true>));