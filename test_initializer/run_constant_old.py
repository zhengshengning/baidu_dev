...
def run_one_case(shape, value, dtype_str, device="gpu"):
    torch_dtype = DTYPE_MAP[dtype_str]

    set_deterministic(SEED)
    pd_out = paddle_constant_init(shape, value=value, dtype=dtype_str, device=device)

    set_deterministic(SEED)
    th_out = torch_constant_init(shape, value=value, dtype=torch_dtype, device=device)
...
if __name__ == "__main__":
    ...
    for dtype_str in DTYPES:
        for shape in SHAPES:
            for value in VALUES:
                try:
                    r = run_one_case(shape, value, dtype_str, device)
                    r["status"] = "ok"
    ...
    if nonzero:
        ...
        print(
            ...
            f"value={worst['value']} -> max={worst['max_abs_diff']:.6g}"
        )
    ...
    if err:
        print("\nErrors:")
        for r in err:
            print(f"  dtype={r['dtype']} shape={r['shape']} value={r['value']} -> {r['status']}")

    # ------------ Save results to CSV ------------
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "run_constant_old.csv")
    fields = [
        "dtype", "shape", "value", "numel",
