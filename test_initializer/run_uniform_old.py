...
def run_one_case(shape, low, high, dtype_str, device="gpu"):
    torch_dtype, _ = DTYPE_MAP[dtype_str]

    set_deterministic(SEED)
    pd_out = paddle_uniform_init(
        shape, low=low, high=high, dtype=dtype_str, device=device
    )

    set_deterministic(SEED)
    th_out = torch_uniform_init(
        shape, low=low, high=high, dtype=torch_dtype, device=device
    )
    ...
    return {
        ...
        "pd_min": float(pd_out.min()),
        "pd_max": float(pd_out.max()),
...
if __name__ == "__main__":
    ...
    for dtype_str in DTYPES:
        for shape in SHAPES:
            for (low, high) in RANGES:
                try:
                    r = run_one_case(shape, low, high, dtype_str, device)
                    r["status"] = "ok"
    ...
    if nonzero:
        ...
        print(
            ...
            f"range={worst['range']} -> max={worst['max_abs_diff']:.6g}"
        )
    ...
    if err:
        print("\nErrors:")
        for r in err:
            print(f"  dtype={r['dtype']} shape={r['shape']} range={r['range']} -> {r['status']}")

    # ------------ Save results to CSV ------------
    csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "run_uniform_old.csv")
    fields = [
        "dtype", "shape", "low", "high", "numel",
