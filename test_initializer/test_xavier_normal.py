...
def paddle_xavier_normal_init(
    ...
    """
    Directly call paddle._C_ops.gaussian with Xavier-normal std.
    Equivalent to paddle.nn.initializer.XavierNormal in dynamic graph mode.

    XavierNormal computes:
        std = gain * sqrt(2 / (fan_in + fan_out))
    For a 2D weight of shape [in_features, out_features], fan_in/fan_out are
    derived from shape[0]/shape[1] (matching XavierInitializer._compute_fans
    for 2D tensors).
    """
