...
def benchmark_paddle_vs_torch_layer_norm():
    ...
    # hidden_sizes = [768,1024,1536,2304, 3072,3840,4096,5120,6144,8192,10240,12288,12800,14336,15360,16384,18432,20480,24576,25600,30720,32768]
    hidden_sizes = [768]
    seq_lens = [32]
    dtypes = ['bfloat16']
    epsilon = 1e-5
    ...
    for hidden_size in hidden_sizes: # H
        for batch_size in batch_sizes: # B
            for seq_len in seq_lens: # C
                print(f"\n测试配置: batch_size={batch_size}, hidden_size={hidden_size}, seq_len={seq_len}")
                
                # 生成基准数据 (float32)
                x_np = np.random.randn(batch_size, seq_len, hidden_size).astype('float32')
                scale_np = np.ones(hidden_size).astype('float32')
                
                for dtype in dtypes:
                    print(f"  [Dtype: {dtype}, batch_size: {batch_size}, seq_len: {seq_len}, hidden_size: {hidden_size}]")
                    try:
                        # === Paddle 准备 ===
                        ...
                        for _ in range(500):
                            ...
                            _ = torch.nn.functional.rms_norm(x_torch, (hidden_size,), weight_torch, eps=epsilon)
                        ...
                        start = time.time()
                        for _ in range(1000):
                            y_torch = torch.nn.functional.rms_norm(x_torch, (hidden_size,), weight_torch, eps=epsilon)
                        torch.cuda.synchronize()
                        end = time.time()
