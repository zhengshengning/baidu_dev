...
def benchmark_paddle_vs_torch_layer_norm():
    ...
    # hidden_sizes = [766,1534,2302,3842,4094,8190,10242,15362,20482,25602,30722]
    batch_sizes = [200]
    hidden_sizes = [2048]
    seq_lens = [32]
    dtypes = ['float32']
    ...
    for hidden_size in hidden_sizes: # H
        for batch_size in batch_sizes: # B
            for seq_len in seq_lens: # C
                ...
                scale_np = np.random.randn(hidden_size).astype('float32')
                ...
                for dtype in dtypes:
                    ...
                    try:
                        ...
                        start = time.time()
                        for _ in range(1):
                            y_paddle = paddle.nn.functional.rms_norm(x_paddle, (hidden_size,), None, eps=epsilon)
                        ...
                        start = time.time()
                        for _ in range(1):
                            y_torch = torch.nn.functional.rms_norm(x_torch, (hidden_size,), None, eps=epsilon)
                        torch.cuda.synchronize()
                        end = time.time()
