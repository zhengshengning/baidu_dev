...
def test_topk(top_k, np_router_logits):
    ...
    pd_router_top_value = pd_router_top_value.astype("float32").detach().cpu().numpy()
    ...
    torch_router_top_value = torch_router_top_value.float().detach().cpu().numpy()
    np_torch_router_indices = torch_router_indices.float().detach().cpu().numpy()

    # print("【top_value】")
    # print("paddle: ", type(pd_router_top_value), pd_router_top_value.shape, "\n", pd_router_top_value)
    # print("torch: ", type(torch_router_top_value), torch_router_top_value.shape, "\n", torch_router_top_value)
    ...
    # assert max_abs == 0 and mean_abs == 0
...
if __name__ == "__main__":
    ...
    np_router_logits = (np.ones((2048)).astype("int32"))
    test_topk(8, np_router_logits)

    # for i in range(1, 33):
    #     np_router_logits = (np.ones((128 * i)).astype("int32"))
    #     test_topk(8, np_router_logits)
